...
def validate_base_seq(seq: str, RNAflag = False) -> bool:
    ...
    bases = 0
    if RNAflag == True:
        for char in RNA_bases:
            bases += seq.count(char)
    else:
        for char in DNA_bases:
            bases += seq.count(char)
    return bases == len(seq)
...
def oneline_fasta(input_file, output_file):
    '''Converts a multi-line fasta sequence to one line, so each read is one header line and one sequence line'''
...
RNA_complements = DNA_complements.copy()
RNA_complements["A"] = "U"

def reverse_comp(input_seq:str, RNAoutput = False): 
    '''Takes a DNA string and returns the reverse complement DNA (or RNA if RNAoutput == True)'''
    if validate_base_seq(input_seq) == False:
        raise TypeError("Input string is not a DNA sequence")
    reverse = input_seq[::-1]
    complement = ""
    for char in reverse:
        if RNAoutput == True:
            complement = complement + (RNA_complements[char])
        else:
            complement = complement + (DNA_complements[char])
    return complement
